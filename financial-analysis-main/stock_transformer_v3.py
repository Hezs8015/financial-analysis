...
class AdaptiveAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # 投影到多头空间
        q = self.q_proj(x).view(B, T, self.n_heads, self.d_head).transpose(1, 2)  # (B, n_heads, T, d_head)
        k = self.k_proj(x).view(B, T, self.n_heads, self.d_head).transpose(1, 2)  # (B, n_heads, T, d_head)
        v = self.v_proj(x).view(B, T, self.n_heads, self.d_head).transpose(1, 2)  # (B, n_heads, T, d_head)
        ...
        if mask is not None:
            # 正确处理掩码维度：[1, 1, T, T]
            attn_scores = attn_scores.masked_fill(mask.unsqueeze(0).unsqueeze(0), -torch.inf)
        
        # 计算基础注意力权重
        attn_weights = F.softmax(attn_scores, dim=-1)  # (B, n_heads, T, T)
        
        # 计算自适应门控权重
        gate_weights = self.attention_gate(x.mean(dim=1))  # (B, n_heads)
        gate_weights = gate_weights.unsqueeze(-1).unsqueeze(-1)  # (B, n_heads, 1, 1)
        
        # 应用门控权重
        attn_weights = attn_weights * gate_weights  # (B, n_heads, T, T)
